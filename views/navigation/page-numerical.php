...
<nav class="page-navigation" data-page-navigation>
	<ul class="nav-list pagination-list pagination-numerical">
		<?php if ( $getPage != 1 ) { ?>
		<li>
			<a class="page-end page-first" href="<?php echo blog_url(); ?>?page=1"><span class="page-icon"><?php echo $first_icon; ?></span> <?php $L->p( 'First' ); ?></a>
		</li>
		<?php } ?>
		...
		<?php if ( $getPage >= 3 ) { ?>
		<li>
			<a href="<?php echo blog_url(); ?>?page=<?php echo $getPage - 2; ?>"><?php echo $getPage - 2; ?></a>
		</li>
		<?php } ?>
		<?php if ( $getPage >= 2 ) { ?>
		<li>
			<a href="<?php echo blog_url(); ?>?page=<?php echo $getPage - 1; ?>"><?php echo $getPage - 1; ?></a>
		</li>
		<?php } ?>
		<?php if ( $getPage ) { ?>
		<li>
			<a class="page-active" href="<?php echo blog_url().'?page='.$getPage; ?>"><?php echo $getPage; ?></a>
		</li>
		<?php } ?>
		<?php if ( Paginator :: numberOfPages() - 1 >= $getPage ) { ?>
		<li>
			<a href="<?php echo blog_url().'?page='.$getPage + 1; ?>"><?php echo $getPage + 1; ?></a>
		</li>
		<?php } ?>
		<?php if ( Paginator :: numberOfPages() - 2 >= $getPage ) { ?>
		<li>
			<a href="<?php echo blog_url().'?page='.$getPage + 2; ?>"><?php echo $getPage + 2; ?></a>
		</li>
		<?php } ?>
		...
		<li>
			<a class="page-end page-last" href="<?php echo blog_url().'?page=' . Paginator :: numberOfPages(); ?>"><?php $L->p( 'Last' ); ?> <span class="page-icon"><?php echo $last_icon; ?></span></a>
		</li>
		<?php } ?>
